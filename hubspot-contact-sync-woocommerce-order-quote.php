...
function hubspot_sync_order_contacts( $order_id ) {
    ...
    $page_url = site_url('/');
    $page_name = "Home page";
    $lead_source_description = 'PEL - RFQ/Order';
  ...
  if ( $order->has_status('ywraq-new') ) {
      ...
      $lead_source_description = 'PEL - RFQ/Quote';
  ...
  // $order_billing_state = isset($order->get_billing_state())?$order->get_billing_state():$order_data['billing']['state'];
  // $order_billing_postcode = isset($order->get_billing_postcode())?$order->get_billing_postcode():$order_data['billing']['postcode'];
  ...
  $order_billing_state_short = $order->get_billing_state();
  $order_billing_postcode = $order->get_billing_postcode();
  $order_billing_country_short = $order->get_billing_country();

  $order_billing_state = WC()->countries->states[$order->get_billing_country()][$order->get_billing_state()];
  $order_billing_country = WC()->countries->countries[$order->get_billing_country()];

  if($order_billing_country_short == 'UK' || $order_billing_country_short == 'GB'){
      $order_billing_country = 'United Kingdom';
  }else if($order_billing_country_short == 'USA' || $order_billing_country_short == 'US'){
      $order_billing_country = 'United States';
  }
  ...
  /* $hs_context stores analytics data about your visitor, including 
    ...
    $fields = array(
      ...
      'leadsource' => 'Website',
      'lead_source_description_c' => $lead_source_description,
    ...
    $portalId = '123594';
    // $portalId = '7760466';
    $formGuid = '48ed3a36-f7c4-42fc-9ea1-23808212f555';
